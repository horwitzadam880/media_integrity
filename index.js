...
const { Observable, forkJoin, defer, timer, from, fromEvent, lastValueFrom, throwError, ReplaySubject } = require('rxjs');
const { filter, take, map, retry, delayWhen, tap, toArray, timeout } = require('rxjs/operators');

const BROCK_FOOTAGE_URL = "https://www.dropbox.com/scl/fi/soy9tt49p0x7eyoohjjpm/Brock-s-bodycam.mp4?rlkey=v6prev2pzm0b8axpltjcxczms&e=3&st=lhmaq952&dl=0"
...
async function main() {
    ...
    const m3u8$ = response$.pipe(
        filter(res => (res.url().includes('.m3u8') ||
            res.url().includes('m3u8') ||
            (res.headers()['content-type'] || '').includes('mpegurl')) && res.status() === 200),
        timeout({
            each: 10000,
